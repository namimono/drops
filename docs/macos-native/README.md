...
> 文档版本：V1.0  
> 制定日期：2026-07-13  
> 需求基线：[Drops-PRD.md](../Drops-PRD.md)  
> 方案状态：待评审
...
PRD 第 8 章列出的待确认项需要在开发对应功能前完成产品决策，尤其包括：

- 压缩、归档等操作究竟处理全部兼容内容还是仅处理选中内容。
- 不同处理工具的默认输出位置是否需要统一。
- 临时内容的清理时机和保留承诺。
- 中文、英文或完整本地化策略。
...
flowchart TD
    ...
    Domain --> Persistence["Temporary Content / Preferences"]
```
...
```swift
struct ShelfID: Hashable, Sendable {}
...
| CLI 参数和输出规则 | 作为 Swift 任务服务的行为参考 |
...
现有压缩、归档和下载能力主要由 Dart 组织外部命令和文件处理。迁移时建议定义统一协议：
...
初期可以通过 Swift `Process` 调用现有可执行工具，保持参数和输出规则一致；后续再决定是否替换为原生库。不要为了原生化同时重写所有媒体编解码能力。

## 8. 实施阶段

### 阶段 0：需求冻结与技术验证

目标：确认 V1 行为并验证最关键的系统能力。

- 评审 PRD 第 8 章待确认项。
- 为 PRD 第 9 章核心场景补充 Given/When/Then 验收步骤。
- 建立独立 Xcode 工程、测试 Target 和产品 Bundle ID 配置。
- 完成无边框浮窗、Finder 拖入、跨应用拖出和不抢焦点窗口的技术验证。

退出条件：关键 AppKit 能力验证通过，不存在阻断架构选择的问题。

### 阶段 1：内容架领域与窗口骨架

- 实现 `Shelf`、`ShelfItem`、生命周期和选择模型。
- 实现 `ShelfManager` 和多窗口管理。
- 实现快捷键、菜单创建、关闭和最多 20 个窗口限制。
- 实现空内容架、收起态和展开态基本界面。

退出条件：可以稳定创建、展开、收起和关闭多个独立内容架。
...
- 实现临时内容落盘、去重和排序。
- 实现单项、多选、范围选择和整堆拖出。
- 实现摇动唤起、拖拽会话和临时内容架晋升/关闭。

退出条件：PRD 9.1 与 9.2 的基础拖放和粘贴场景通过。

### 阶段 3：原生交互完善

- 网格/列表切换、滚动、文件图标和缩略图。
- 打开、Finder 定位、Quick Look 和链接预览。
- 右键菜单、移除和文本合并。
- 窗口和内容动画、键盘操作、焦点和多屏约束。
- 菜单栏、基本设置、日志和关于页。

退出条件：核心版本具备完整的日常使用能力。

### 阶段 4：文件处理能力迁移

- 建立统一任务、进度、取消、错误和重试模型。
- 迁移压缩和裁剪。
- 迁移归档。
- 迁移音频、ICO 和下载工具。
- 迁移能力检查与首次准备流程。

退出条件：PRD 9.3 与 9.4 场景通过。

### 阶段 5：发布准备

- 完成性能、稳定性、内存、权限和多屏验证。
- 完成 Bundle ID、签名、Entitlements 和 Sparkle 配置。
- 验证升级安装、设置迁移和旧临时文件清理。
- 从构建、打包和发布流程中移除 Flutter 与 Windows 入口。

退出条件：原生版本满足发布清单，且核心数据和设置迁移方案完成验证，仓库只产出 macOS 原生应用。
...
| 风险 | 控制措施 |
|---|---|
| 重写范围过大导致长期不可用 | 先交付内容架 MVP，再迁移高级处理功能 |
| 把旧实现缺陷复制到新版本 | PRD 和验收用例优先，旧代码只作参考 |
| 全 SwiftUI 再次遇到拖放和窗口限制 | AppKit 承担核心窗口和拖放，SwiftUI 仅用于普通页面 |
| 文件处理能力迁移耗时 | 初期继续使用外部工具，先迁移调度层 |
| 一次性移除旧工程导致参考能力丢失 | 先迁移规则、资源和工具参数，再从构建流程与仓库中清理旧代码 |
| 发布配置迁移影响现有用户 | 单独验证 Bundle ID、签名、设置迁移和 Sparkle 更新链路 |
| 临时文件泄漏或误删原文件 | 独立临时目录、来源标记、引用计数和清理测试 |
| 全局事件权限或系统版本差异 | 阶段 0 在最低支持系统和无额外权限环境验证 |

## 12. 下一步

1. 评审并确认本文的工程决策。
2. 对 PRD 第 8 章逐项作出产品决定，形成 V1 范围。
3. 把 PRD 第 9 章改写为可执行验收清单。
4. 创建 `macos-native/Drops.xcodeproj`、测试 Target 和产品 Bundle ID 配置。
5. 用最小技术样例验证浮窗、拖入、拖出、焦点和摇动唤起。
